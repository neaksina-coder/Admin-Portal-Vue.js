...
const downloadInvoice = async () => {
  try {
    const response = await $api.get(`/apps/invoice/${route.params.id}/download`, { responseType: 'blob' })
    const url = window.URL.createObjectURL(new Blob([response]))
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', `invoice-${route.params.id}.pdf`) // Or whatever the file name should be
